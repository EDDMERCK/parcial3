...
public class GUIManual extends JFrame {
    ...
    public void iniciodesesion() {
        int validado = 0;
        while (validado == 0){
            String opciones[] = {"Inicial sesion", "Registrate", "Salir"};
            int opcion = 0;

            opcion = JOptionPane.showOptionDialog(null, "Inicio de sesion", "login", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, opciones, 0);

            if (opcion == 0) {
                String usuario, contrasena;
                usuario = JOptionPane.showInputDialog("Introduce tu nombre de usuario");
                contrasena = JOptionPane.showInputDialog("Introduce tu contraseña");

                //combierte el nombre del usuario a minuscula
                for (int i = 0; i < usuarios.length; i++) {
                    if (usuarios[i][0].equals(usuario) && usuarios[i][1].equals(contrasena)) {
                        haySesion = true;
                        validado = 1;
